...
namespace Brumba.DwaNavigator.Tests
{
    ...
    public class DynamicDiamondGeneratorTests
    {
        ...
        public void Generate()
        {
            ...
            Assert.That(dd.Cast<VelocityAcceleration>().Count(v => Math.Abs(v.Velocity.Linear - (3 + accMax.Linear * 0.1 / DynamicDiamondGenerator.STEPS_NUMBER)) < 0.0001), Is.EqualTo(2 * DynamicDiamondGenerator.STEPS_NUMBER + 1 - 2));
            Assert.That(dd.Cast<VelocityAcceleration>().Count(v => Math.Abs(v.Velocity.Angular - accMax.Angular * 0.1 / DynamicDiamondGenerator.STEPS_NUMBER) < 0.0001), Is.EqualTo(2 * DynamicDiamondGenerator.STEPS_NUMBER + 1 - 2));
        }
    }
}